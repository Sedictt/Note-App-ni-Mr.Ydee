      ...
      <div className="bg-gray-100 rounded-xl shadow-xl w-full max-w-5xl max-h-[90vh] flex flex-col" onClick={e => e.stopPropagation()}>
        ...
        <div className="flex-grow p-4 overflow-y-auto flex flex-col lg:flex-row gap-6">
          <div className="lg:w-1/3 space-y-4">
            ...
            <div>
             ...
             <button
                ...
                className="w-full flex items-center justify-center bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-3 px-4 rounded-lg shadow-md hover:shadow-lg transition-all duration-300 disabled:bg-gray-400 disabled:cursor-wait"
            >
          ...
          <div className="lg:w-2/3 flex-shrink-0 flex justify-center items-center bg-gray-200 p-2 sm:p-4 rounded-lg">
            <div ref={exportRef} className={`${aspectRatios[ratio].class} w-full bg-gradient-to-br from-slate-800 to-slate-900 text-white p-6 md:p-8 lg:p-10 flex flex-col font-sans`}>
                <header className="flex justify-between items-start border-b border-slate-600 pb-4">
                    <div>
                        <h1 className="text-2xl md:text-3xl lg:text-4xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-sky-300 to-indigo-300 tracking-tight">Upcoming Tasks</h1>
                ...
                <div className="flex-grow mt-6 space-y-3 lg:space-y-4 overflow-y-auto pr-2 custom-scrollbar">
                    {tasks.map(task => (
                        <div key={task.id} className="bg-slate-700/60 rounded-lg p-3 lg:p-4 flex items-center gap-4 border border-slate-600">
                            <div className="flex-shrink-0">{categoryIcons[task.category]}</div>
                            ...
                            <div className={`text-xs font-bold px-2.5 py-1 rounded-full flex-shrink-0 ${task.priority === 'High' ? 'bg-red-500/80 text-white' : task.priority === 'Medium' ? 'bg-yellow-500/80 text-yellow-950' : 'bg-green-500/80 text-white'}`}>
                                {task.priority}
                            </div>
                        </div>
                    ))}
                ...
                <footer className="mt-auto pt-4 text-center text-xs text-slate-500/80">
